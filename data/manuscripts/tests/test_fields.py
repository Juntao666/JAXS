...
def test_get_verdicts():
    assert isinstance(mflds.get_verdicts(), list)
...
@patch('data.manuscripts.fields.get_history', autospec=True, return_value=['SUB', 'REJ'])
...
@patch('data.manuscripts.fields.get_history', autospec=True, return_value=['INVALID_STATE'])
...
@patch('data.manuscripts.fields.get_referees', autospec=True, return_value={
    'referee1': ['ACC'],
    'referee2': ['AWR'],
    'referee3': ['REJ'],
})
